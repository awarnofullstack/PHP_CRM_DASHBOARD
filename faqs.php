...
<div class="col-10 main-section bg-white border p-2">
   ...
   <div class="dn-form-container">
      ...
      if (isset($_GET['id'])) {
         ...
         $query = mysqli_query($conn, "select * from faqs where id = $find_id");
      ...
      if (isset($_GET['delete'])) {
         ...
         $query = mysqli_query($conn, "delete from faqs where id = $find_id");

         if ($query) {
            set_ss("faqs deleted with id = $find_id successfuly", $_SERVER['PHP_SELF']);
         } else {
            set_ss("faqs delete with id = $find_id failed", $_SERVER['PHP_SELF'], 'error');
         }
      }
      ...
      if (isset($_POST['save'])) {
         ...
         $query = mysqli_query($conn, "insert into faqs (title,content) values ('$title','$content')");

         if ($query) {
            set_ss('faqs inserted successfully', $_SERVER['PHP_SELF']);
         } else {
            set_ss('faqs failed to insert', $_SERVER['PHP_SELF'], 'error');
         }
      }
      ...
      if (isset($_POST['edit'])) {
         ...
         if ($_FILES['content']['name']) {
            ...
            $query = mysqli_query($conn, "UPDATE faqs SET title='$title' , content='$content' WHERE id = $id");
         } else {
            $query = mysqli_query($conn, "UPDATE faqs SET title = '$title',content = '$content' WHERE id = $id");
         }

         if ($query) {
            set_ss('faqs updated successfully', $_SERVER['PHP_SELF']);
         } else {
            set_ss('faqs failed to update'.$id.mysqli_error($conn), $_SERVER['PHP_SELF'], 'error');
         }
      }
   ...
   <div class="dn-form-container">
      ...
      <table class="table">
         ...
         <?php
         /* check if data exist */
         $select = mysqli_query($conn, "select * from faqs");
         $inc = 0;
