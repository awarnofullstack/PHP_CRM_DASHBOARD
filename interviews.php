...
<div class="col-10 main-section bg-white border p-2">
   <h5 class="css">Manage Interviews</h5>
   ...
   <div class="dn-form-container">
      ...
      if (isset($_GET['id'])) {
         ...
         $query = mysqli_query($conn, "select * from interviews where id = $find_id");
      ...
      if (isset($_GET['delete'])) {
         ...
         $query = mysqli_query($conn, "delete from interviews where id = $find_id");

         if ($query) {
            set_ss("Interview deleted with id = $find_id successfuly", $_SERVER['PHP_SELF']);
         } else {
            set_ss("Interview delete with id = $find_id failed", $_SERVER['PHP_SELF'], 'error');
         }
      }
      ...
      if (isset($_POST['save'])) {
        ...
        //  $content = htmlspecialchars($_POST['content']);
         ...
         $query = mysqli_query($conn, "insert into interviews (title,logo,url) values ('$title','$path','$url')");

         if ($query) {
            set_ss('Interview inserted successfully', $_SERVER['PHP_SELF']);
         } else {
            set_ss('Interview failed to insert'.mysqli_error($conn), $_SERVER['PHP_SELF'], 'error');
         }
      }
      ...
      if (isset($_POST['edit'])) {
        ...
        //  $content = htmlspecialchars($_POST['content']);
         ...
         if ($_FILES['logo']['name']) {
            ...
            $query = mysqli_query($conn, "UPDATE interviews SET title='$title',url='$url',logo='$path' WHERE id = $id");
         } else {
            $query = mysqli_query($conn, "UPDATE interviews SET title = '$title',url='$url' WHERE id = $id");
         }

         if ($query) {
            set_ss('Interview updated successfully', $_SERVER['PHP_SELF']);
         } else {
            set_ss('Interview failed to update'.$id.mysqli_error($conn), $_SERVER['PHP_SELF'], 'error');
         }
      }
      ...
      <?php isset($_GET['id']) ? '?id=' . $_GET['id'] : ''; ?>" method="POST" id="class-form" enctype="multipart/form-data">
         ...
         <div class="mb-3">
            ...
            <input type="text" name="url" value="<?php echo $url; ?>" required class="form-control" id="url" placeholder="type....">
         </div>
   ...
   <div class="dn-form-container">
      ...
      <table class="table">
         ...
         <?php
         /* check if data exist */
         $select = mysqli_query($conn, "select * from interviews");
         $inc = 0;
