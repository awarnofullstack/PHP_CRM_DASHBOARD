...
<div class="col-10 main-section bg-white border p-2">
    ...
    <div class="dn-form-container">
        ...
        if (isset($_GET['id'])) {
            ...
            $query = mysqli_query($conn, "select * from menu where id = $find_id");
        ...
        if (isset($_GET['delete'])) {
            ...
            $query = mysqli_query($conn, "delete from menu where id = $find_id");

            if ($query) {
                set_ss("menu deleted with id = $find_id successfuly", $_SERVER['PHP_SELF']);
            } else {
                set_ss("menu delete with id = $find_id failed", $_SERVER['PHP_SELF'], 'error');
            }
        }
        ...
        if (isset($_POST['save'])) {
            ...
            $query = mysqli_query($conn, "insert into menu (title,logo) values ('$title','$path')");

            if ($query) {
                set_ss('menu inserted successfully', $_SERVER['PHP_SELF']);
            } else {
                set_ss('menu failed to insert', $_SERVER['PHP_SELF'], 'error');
            }
        }
        ...
        if (isset($_POST['edit'])) {
            ...
            if ($_FILES['logo']['name']) {
                ...
                $query = mysqli_query($conn, "UPDATE menu SET title='$title',logo='$path' WHERE id = $id");
            } else {
                $query = mysqli_query($conn, "UPDATE menu SET title = '$title' WHERE id = $id");
            }

            if ($query) {
                set_ss('menu updated successfully', $_SERVER['PHP_SELF']);
            } else {
                set_ss('menu failed to update' . $id . mysqli_error($conn), $_SERVER['PHP_SELF'], 'error');
            }
        }
