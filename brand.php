...
<div class="col-10 main-section bg-white border p-2">
    <h5 class="css">Brand partners</h5>

    <!-- main header content  -->
    <div class="dn-form-container">
        <div class="d-flex justify-content-between top">
            <h3>Add Brand Partners</h3>
        ...
        if (isset($_GET['id'])) {
            ...
            $query = mysqli_query($conn, "select * from brands where id = $find_id");
        ...
        if (isset($_GET['delete'])) {
            ...
            $query = mysqli_query($conn, "delete from brands where id = $find_id");

            if ($query) {
                set_ss("brands deleted with id = $find_id successfuly", $_SERVER['PHP_SELF']);
            } else {
                set_ss("brands delete with id = $find_id failed", $_SERVER['PHP_SELF'], 'error');
            }
        }
        ...
        if (isset($_POST['save'])) {
            ...
            $query = mysqli_query($conn, "insert into brands (title,logo,link) values ('$title','$path' ,'$url')");

            if ($query) {
                set_ss('brands inserted successfully', $_SERVER['PHP_SELF']);
            } else {
                set_ss('brands failed to insert', $_SERVER['PHP_SELF'], 'error');
            }
        }
        ...
        if (isset($_POST['edit'])) {
            ...
            if ($_FILES['logo']['name']) {
                ...
                $query = mysqli_query($conn, "UPDATE brands SET title='$title',link='$url',logo='$path' WHERE id = $id");
            } else {
                $query = mysqli_query($conn, "UPDATE brands SET title = '$title',link='$url' WHERE id = $id");
            }

            if ($query) {
                set_ss('brands updated successfully', $_SERVER['PHP_SELF']);
            } else {
                set_ss('brands failed to update ' . $id . mysqli_error($conn), $_SERVER['PHP_SELF'], 'error');
            }
        }
    ...
    <div class="dn-form-container">
        ...
        <table class="table">
            ...
            <?php
            /* check if data exist */
            $select = mysqli_query($conn, "select * from brands");
            $inc = 0;
