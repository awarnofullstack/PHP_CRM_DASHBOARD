...
<div class="col-10 main-section bg-white border p-2">
    <h5 class="css">Our Catalog</h5>

    <!-- main header content  -->
    <div class="dn-form-container">
        <div class="d-flex justify-content-between top">
            <h3>Add Catalog Images</h3>
        ...
        if (isset($_GET['id'])) {
            ...
            $query = mysqli_query($conn, "select * from catalog where id = $find_id");
        ...
        if (isset($_GET['delete'])) {
            ...
            $query = mysqli_query($conn, "delete from catalog where id = $find_id");

            if ($query) {
                set_ss("catalog deleted with id = $find_id successfuly", $_SERVER['PHP_SELF']);
            } else {
                set_ss("catalog delete with id = $find_id failed", $_SERVER['PHP_SELF'], 'error');
            }
        }
        ...
        if (isset($_POST['save'])) {
            ...
            $query = mysqli_query($conn, "insert into catalog (title,logo) values ('$title','$path')");

            if ($query) {
                set_ss('catalog inserted successfully', $_SERVER['PHP_SELF']);
            } else {
                set_ss('catalog failed to insert', $_SERVER['PHP_SELF'], 'error');
            }
        }
        ...
        if (isset($_POST['edit'])) {
            ...
            if ($_FILES['logo']['name']) {
                ...
                $query = mysqli_query($conn, "UPDATE catalog SET title='$title',logo='$path' WHERE id = $id");
            } else {
                $query = mysqli_query($conn, "UPDATE catalog SET title = '$title' WHERE id = $id");
            }

            if ($query) {
                set_ss('catalog updated successfully', $_SERVER['PHP_SELF']);
            } else {
                set_ss('catalog failed to update' . $id . mysqli_error($conn), $_SERVER['PHP_SELF'], 'error');
            }
        }
      ...
      <?php isset($_GET['id']) ? '?id=' . $_GET['id'] : ''; ?>" method="POST" id="class-form" enctype="multipart/form-data">
            <div class="mb-3">
                ...
                <select type="text" name="title" value="<?php echo $title; ?>" required class="form-control" id="title" placeholder="type....">
                    <option selected="">...</option>
                    <option value="easy bites">easy bites</option>
                    <option value="big bites">big bites</option>
                    <option value="signature bebides">signature bebides</option>
    ...
    <div class="dn-form-container">
        ...
        <table class="table">
            ...
            <?php
            /* check if data exist */
            $select = mysqli_query($conn, "select * from catalog");
            $inc = 0;
